...
use FastLaravel\Http\Database\Exception\ExceptionHandlerInterface;
...
class Connection implements ConnectionInterface
{
     ...
     * @return mixed
     */
    public function selectOne($query, array $bindings = array(), $useReadPdo = true)
    {
        return $this->fetch($query, $bindings, $useReadPdo);
    }
    ...
    {
        return $this->fetchAll($query, $bindings, $useReadPdo);
    }
    ...
    private function execute($query, $bindings, $useReadPdo)
    {
        ...
        while ($retry--) {
            ...
            // if there is one, or throw the exception if not
                ...
                $code = $errorInfo[1] ?? 0;
                if (in_array($code, ['2006', '2013']) && $retry) {
                    $this->reconnect();
                    continue;
                }
